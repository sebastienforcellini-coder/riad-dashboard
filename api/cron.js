...
export default async function handler(req, res) {
  ...
  try {
    ...
    // 4. Fusionner intelligemment — NE JAMAIS écraser les montants existants
    ...
    const airbnb   = newB.map(b => ({
      ...b,
      amount: existing[b.id]?.amount ?? 0,  // Préserver montant existant
      name:   existing[b.id]?.name   ?? "",
      guests: existing[b.id]?.guests ?? "",
      paid:   existing[b.id]?.paid   ?? false,
    }));

    // Blocages : garder les perso, filtrer Airbnb en excluant les résa manuelles
